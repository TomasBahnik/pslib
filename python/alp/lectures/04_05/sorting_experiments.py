...
def time_sorting_algorithms(sorted=False):
    ...
    plt.savefig("time_sorting_algorithms_sorted.pdf" if sorted else "time_sorting_algorithms.pdf")
    plt.figure(2)
    for i in range(len(algs)):
        plt.loglog(ns, alltimes[i], marker='o', linewidth=3, label=algs[i].__name__)
    plt.legend(loc='upper left')
    plt.savefig("time_sorting_algorithms_sorted_log.pdf" if sorted else "time_sorting_algorithms_log.pdf")
    plt.show()
